...
def balance_assets(balance, percent_per_asset):
    ...
    for asset in balance:
        if asset != 'ZUSD':
            ...
            if (percent_per_asset[asset] + GLOBAL_BALANCE_DIFF_THRESHOLD < balance[asset]['percent']):
                type = 'sell'
            elif (percent_per_asset[asset] > balance[asset]['percent'] + GLOBAL_BALANCE_DIFF_THRESHOLD):
                type = 'buy'
            else:
                continue
