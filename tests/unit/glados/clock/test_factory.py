...
class TestClockConfig:
    ...
    def test_is_backtest_true_with_times(self):
        ...
        config = ClockConfig(
            ...
            backtest_end=datetime(2025, 1, 31, tzinfo=UTC),
        )
    ...
    def test_raises_if_end_before_start(self):
        ...
        with pytest.raises(ValueError, match="backtest_end must be >= backtest_start"):
            ClockConfig(
                backtest_start=datetime(2025, 1, 31, tzinfo=UTC),
                backtest_end=datetime(2025, 1, 1, tzinfo=UTC),
            )
...
class TestCreateClock:
    ...
    def test_passes_time_range_to_backtest(self):
        ...
        end = datetime(2025, 1, 1, 16, 0, tzinfo=UTC)
        config = ClockConfig(
            backtest_start=start,
            backtest_end=end,
        )
