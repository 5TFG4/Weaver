...
class TestControllableClock:
    ...
    def test_clock_advance(self) -> None:
        ...
        assert ticks[0].ts == datetime(2024, 1, 15, 9, 31, tzinfo=UTC)
        assert ticks[1].ts == datetime(2024, 1, 15, 9, 32, tzinfo=UTC)
        assert ticks[2].ts == datetime(2024, 1, 15, 9, 33, tzinfo=UTC)
