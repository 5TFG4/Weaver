...
## Appendix C: Logical Consistency Audit (Post-M3)

> This section documents design-vs-implementation mismatches discovered during the post-M3 audit.

### C.1 Issues Fixed (M3 Hotfix)

| Issue | Description | Fix Applied |
|-------|-------------|-------------|
| **Empty lifespan** | `app.py` lifespan was a pass-through, did not initialize DB/EventLog | Added full lifespan: Database.init, EventLog.start, SSE subscription |
| **VedaService missing** | Design spec'd a VedaService orchestrator, but only OrderManager existed | Created `src/veda/veda_service.py` with VedaService class |
| **OrderManager isolation** | OrderManager not connected to OrderRepository or EventLog | VedaService integrates OrderManager + OrderRepository + EventLog |
| **EventLog→SSE disconnected** | PostgresEventLog emits events but SSEBroadcaster never receives them | lifespan subscribes EventLog to forward events to SSEBroadcaster |
| **Adapter factory missing** | AlpacaAdapter couldn't be created from AlpacaCredentials | Added `src/veda/adapters/factory.py` with `create_alpaca_adapter()` |

**Files Created**:
- `src/veda/veda_service.py` - VedaService orchestrator + `create_veda_service()` factory
- `src/veda/adapters/factory.py` - `create_alpaca_adapter()`, `create_adapter_for_mode()`

**Files Modified**:
- `src/glados/app.py` - Full lifespan implementation
- `src/veda/__init__.py` - Added new exports
- `src/veda/adapters/__init__.py` - Added factory exports

### C.2 Issues Deferred to M4+

| Issue | Description | Why Deferred | Target Milestone |
|-------|-------------|--------------|------------------|
| **RunManager in-memory** | Run state not persisted to database; lost on restart | MVP-acceptable; documented design limitation | M4 |
| **GLaDOS OrderService mock** | `glados/services/order_service.py` uses dummy data, not VedaService | Waiting for Greta/strategy integration | M4 |
| **CandleStore ephemeral** | Market data not persisted | Backtest will need history; design deferred | M4 |

### C.3 Integration Verification Checklist (M4 Entry Gate)

Before starting M4, verify these integrations work end-to-end:

- [ ] `POST /runs` → creates Run record in database
- [ ] Order placed via REST → persisted in `veda_orders` table
- [ ] EventLog entry → SSE broadcast visible in `/events/stream`
- [ ] Clock tick → triggers strategy (after Marvin integration)
- [ ] Graceful shutdown cleans up all resources

### C.4 Architecture Invariants

These MUST remain true across all milestones:

1. **Single EventLog instance** - All components share one EventLog; no orphan writers
2. **VedaService as entry point** - External callers use VedaService, not OrderManager directly
3. **Database session per request** - No long-lived sessions; use dependency injection
4. **SSE receives all events** - EventLog.subscribe() → SSEBroadcaster pipeline never disconnected
