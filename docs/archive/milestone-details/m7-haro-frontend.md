...
### 6.2 M7-1: React App Scaffold (~8 tests, 1 day)
...
| 1.3 | Install and configure Tailwind CSS | Styles apply | `haro/tailwind.config.js` |
...
export type RunStatus = 'pending' | 'running' | 'stopped' | 'completed' | 'error';
export type OrderSide = 'buy' | 'sell';
export type OrderType = 'market' | 'limit' | 'stop' | 'stop_limit';
export type OrderStatus = 'pending' | 'submitted' | 'accepted' | 'partial' | 'filled' | 'cancelled' | 'rejected' | 'expired';
...
export function useSSE() {
  ...
  const connect = useCallback(() => {
    ...
    eventSource.addEventListener('run.started', (e) => {
      ...
      addNotification({ type: 'success', message: `Run ${data.run_id} started` });
    });
